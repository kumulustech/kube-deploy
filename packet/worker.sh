...
start_k8s() {
    ...
    case "${lsb_dist}" in
        centos)
            if [ "`systemctl show docker | grep EnvironmentFile`" == "" ]; then
		mkdir -p /etc/systemd/system/docker.service.d
                echo -e "[Service]\nExecStart=\nExecStart=/usr/bin/docker daemon -H unix:///var/run/docker.sock  --mtu=${FLANNEL_MTU} --bip=${FLANNEL_SUBNET}" > /etc/systemd/system/docker.service.d/flannel-subnet.conf
